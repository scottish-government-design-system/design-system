 ...
 */
class DSDatePicker extends component_1.default {
    options;
    ...
    init() {
        ...
        this.dialogElement.id = 'datepicker-' + (0, id_modifier_1.default)();
    ...
    setOptions() {
        ...
        if (!this.options.minDate && this.datePickerParent.dataset.mindate) {
            this.options.minDate = this.formattedDateFromString(this.datePickerParent.dataset.mindate, null);
        }
        if (!this.options.maxDate && this.datePickerParent.dataset.maxdate) {
            this.options.maxDate = this.formattedDateFromString(this.datePickerParent.dataset.maxdate, null);
        }
        if (!this.options.disabledDates?.length && this.datePickerParent.dataset.disableddates) {
            this.options.disabledDates = this.datePickerParent.dataset.disableddates
                .replace(/\s+/, ' ')
                .split(' ')
                .map(item => this.formattedDateFromString(item))
                .filter(item => item);
        }
    }
    /**
